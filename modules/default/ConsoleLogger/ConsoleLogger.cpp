/*
 Helix – Console Logger Module
 Purpose:
    - Default stdout/stderr logging sink for the Helix module management platform.
    - Provides simple, timestamped log output for the Helix modules.

 Details:
    - Thread-safety: synchronized via a process-global mutex.
    - Timestamp format: YYYY-MM-DD HH:MM:SS.mmm (local time).
    - Level mapping: 0=DEBUG, 1=INFO, 2=WARN, 3=ERROR.
    - Registration: integrates with helix_log_get_register/unregister.
    - Entry points: logger_init/start/stop/destroy
         Ensure manifest.json entry_points map to these symbols.
    - License: MIT (see repository root LICENSE).

 Notes:
    - This module is intended as a lightweight, dependency-free default logger.
*/
