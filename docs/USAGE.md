...
./helixctl --socket /tmp/helixd.sock install modules/consolelogger.helx
./helixctl --socket /tmp/helixd.sock enable ConsoleLogger
./helixctl --socket /tmp/helixd.sock start ConsoleLogger
...
- Metadata (name, version, description, author) is sourced from `manifest.json`.
...
```bash
mkdir -p modules
./helixd --modules-dir ./modules --socket /tmp/helixd.sock
```
...
Not recommended; use `helixctl` instead.
...
```bash
./helixd --modules-dir ./modules --socket /tmp/helixd.sock --foreground --interactive
```
...
For more help see the [API File](api.md)
